...
describe("task find unique controller", () => {
    ...
    it("should return http status code 404 not found is any task has found", async () => {
        try {
            ...
            await axiosInstance.get(`/task/66d20469-251c-4a46-9dab-728ccddb1ddb`);
        } catch ({ response }: any) {
    ...
    it("should return http status code 200 and user's task", async () => {
        const { status, data } = await axiosInstance.get(`/task/${tempTask.id.value}`);

        expect(status).toBe(HttpStatus.OK);
        expect(data.id.value).toBe(tempTask.id.value);
    });
});